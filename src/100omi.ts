// src/100omi.ts
import { OnePlugin, BaseComment,    } from './types';
      ...
      omikuji: [
        {
          ...
          message: [
            { botKey: 0, iconKey: "joy02", delaySeconds: 1, content: "<<user>>さんの運勢は【大吉】<<random1>>", },
          ],
          party: [{ delaySeconds: 1, content: '!レベルアップ' },],
        },
      ],
      random: [
        { placeholder: "random1", weight: 11, group: 0, content: "人との縁が幸運を呼び込みそう。感謝の気持ちを忘れないことが大事よ。" },
        { placeholder: "random1", weight: 11, group: 0, content: "健康運が特に好調ね。心身ともに充実した日々になるわ。" },
        { placeholder: "random1", weight: 11, group: 0, content: "努力が実を結び、幸運が訪れるって。積極的に行動すると良いことがあるわ。" },
        { placeholder: "random1", weight: 11, group: 0, content: "新しい挑戦が成功をもたらす予感。勇気を出して一歩踏み出してみて。" },
        { placeholder: "random1", weight: 11, group: 0, content: "良い知らせが届くかも。ポジティブな気持ちを持ち続けてね。" },
        { placeholder: "random1", weight: 11, group: 0, content: "困難な状況も乗り越えられるわ。自信を持って進んで大丈夫よ。" }
        ],
  ...
  async filterComment(comment: BaseComment, service, userData) {
    ...
    if (comment.omikenData.isOverlapping === undefined) {
      if (await this.func.checkOverlapping(comment)) {
        comment.omikenData.isOverlapping = true; return comment;
      }
    }
    ...
    for (const rule of this.rules) {
      ...
      if (result && result.isApplicable) {
        ...
        this.functionOmikuji(comment.data.displayName, rule.modes, comment)
          .catch(error => { console.error('おみくじ実行中にエラーが発生しました:', error); });
        break;
      }
    }
