...
export class CommentInstance {
 ...
 // 初期化
 constructor(comment: Comment, visit: VisitType, TimeConfig: TimeConfigType, userData: UserNameData) {
 ...
 omikenSelect(rulesArray: RulesType[], omikujis: Record<string, OmikujiType>): boolean {
 ...
 // 単一ルールの処理
 private ruleProcess(rule: RulesType, omikujis: Record<string, OmikujiType>): OmikujiType | null {
  // thresholdチェック(New)
  const checker = new ThresholdChecker(rule, this.comment, this.visit, this.TimeConfig);
  ...
  const validOmikujis = rule.enableIds
   .map((id) => omikujis[id])
   .filter((omikuji) => {
    const isValidOmikuji = omikuji.threshold.every((threshold) => checker.check(threshold));
 ...
 // おみくじ(アイテム抽選)
 private omikujisLottery(items: OmikujiType[]): OmikujiType | null {
  ...
  // 3. 合計ウェイトを計算
  const totalWeight = filteredItems.reduce((sum, { weight }) => sum + (weight >= 0 ? weight : 1), 0);
  if (totalWeight <= 0) return null;

  // 4. 抽選処理
  let rand = Math.random() * totalWeight;
  return filteredItems.find((item) => (rand -= item.weight >= 0 ? item.weight : 1) < 0) ?? null;
 }
 ...
 async omikujiProcess(
  ...
  // statusがあるなら書き換え
  if (this.selectOmikuji.status) this.visit.status = this.selectOmikuji.status;
  ...
  if (this.selectOmikuji.script) {
   ...
   const result = scriptsCall(Scripts, scriptId, this.comment, this.game, this.visitData, parameter);
   if (result) {
    // placeholderの値を追加
